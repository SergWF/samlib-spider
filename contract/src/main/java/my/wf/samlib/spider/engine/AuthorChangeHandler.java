...
public class AuthorChangeHandler {
    ...
    Writing createWriting(WritingData writingData) {
        ...
        writing.getChanges().clear();
        writing.getChanges().add(Changes.NEW);
        return writing;
    }

    Writing updateWriting(Writing writing, WritingData writingData) {
        writing.getChanges().clear();
        if (!writing.getDescription().equals(writingData.getDescription())) {
            writing.getChanges().add(Changes.DESCRIPTION);
        }
        if (!writing.getSize().equals(writingData.getSize())) {
            writing.getChanges().add(Changes.SIZE);
        }
        if (!writing.getName().equals(writingData.getName())) {
            writing.getChanges().add(Changes.NAME);
        }
    ...
    Optional<Writing> findWritingByData(Author author, WritingData data) {
        return author.getWritings().stream().filter(writing -> writing.getUrl().equals(data.getUrl())).findFirst();
    }

    public void handleDeleted(Author author, Collection<WritingData> writingDatas) {
        Set<String> existingUrls =
                writingDatas.stream().map(writingData -> writingData.getUrl()).collect(Collectors.toSet());
        int removed = author.getWritings().size();
        boolean remove = author.getWritings().removeIf(writing -> !existingUrls.contains(writing.getUrl()));
        removed = removed - author.getWritings().size();
        if (remove) {
    ...
    public Writing handleWritingUpdate(WritingData writingData) {
        ...
        Writing writing = writingByData.isPresent() ? updateWriting(writingByData.get(), writingData) :
                addNewWriting(writingData);
        if (!writing.getChanges().isEmpty()) {
